# Introduction
AutoStream is a GenAI-powered assistant built to help users explore product plans, understand pricing, and get started with the AutoStream platform. Instead of behaving like a normal chatbot that just replies to messages, this project uses an agent-based workflow that follows a clear and structured conversation flow.

The agent guides users step by step — first answering questions about plans and features, then identifying purchase intent, and finally collecting required details such as name, email, and platform before capturing a lead. This makes the interaction feel natural while still ensuring that important business steps are handled correctly.

The project is implemented using LangGraph, which allows the agent to maintain conversation state across multiple messages and decide what action to take next based on user input. AutoStream is designed to be modular, easy to extend, and suitable for real-world deployment scenarios such as customer support chat or WhatsApp-based sales assistants.
